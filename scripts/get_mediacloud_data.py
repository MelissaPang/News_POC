...
def get_random_articles_within_timeframe(mc_client, start_date: datetime, end_date: datetime, num_files: int):
    """
    ...
    random_stories = random.sample(stories, num_files)

    fieldnames = ['stories_id', 'publish_date', 'title', 'url', 'language', 'ap_syndicated', 'themes', 'media_id',
                  'media_name', 'media_url', 'text']
    with open(str(num_files)+'random_articles.csv', 'w', newline='') as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames, extrasaction='ignore')
        writer.writeheader()
        count = 0
        while count <= 3:
            for s in random_stories:
                if s['language'] == "en":
                    writer.writerow(s)
                    count += 1
...
start_date = datetime.date(2021, 3, 22)
end_date = datetime.date(2021, 3, 30)

mc = create_mediacloud_client(my_key)
get_random_articles_within_timeframe(mc, start_date, end_date, 1000)

df = pd.read_csv("/mnt/c/Users/lilly/PycharmProjects/News_Clustering/News_POC/4random_articles.csv")
# url = df['url']

#https://matix.io/extract-text-from-webpage-using-beautifulsoup-and-python/
url = "https://en.wikipedia.org/wiki/Pickles_Auctions"
res = requests.get(url)
html_page = res.content
soup = BeautifulSoup(html_page, 'html.parser')
text = soup.find_all(text=True)
